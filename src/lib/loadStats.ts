...
export function computeLoadStats(load: Load, entries: Entry[]): LoadStats {
  ...
  const tare = (Number(load.container_count) || 0) * (Number(load.weight_per_container) || 0);
  const times = entries.map((e) => e.created_at).filter((t): t is string => !!t).sort();
  return {
...
function fieldValue(obj: Load | Entry, fieldNumber: CatalogFieldNumber): string | null | undefined {
  return fieldNumber === 1 ? obj.custom_field_1 : fieldNumber === 2 ? obj.custom_field_2 : obj.custom_field_3;
}

export function computeLabelGroups(load: Load, entries: Entry[], fieldNumber: CatalogFieldNumber): LabelGroup[] {
...
export function computeNestedGroups(load: Load, entries: Entry[], fields: CatalogFieldNumber[]): NestedGroup[] {
  if (fields.length === 0) return [];
  const [field, ...rest] = fields;
  const loadValue = fieldValue(load, field);
  const groups = new Map<string, { weight: number; count: number; entries: Entry[] }>();
  for (const e of entries) {
  ...
  return [...groups.entries()]
    .map(([value, g]) => ({ value, weight: g.weight, count: g.count, children: rest.length ? computeNestedGroups(load, g.entries, rest) : [] }))
...
export function flattenNestedGroups(groups: NestedGroup[], labels: string[]): BreakdownLine[] {
  ...
  const walk = (nodes: NestedGroup[], depth: number): BreakdownLine[] => {
    ...
    for (const g of nodes) {
      ...
      lines.push({ text, weight: depth === 0 && !showRootWeight ? null : g.weight, depth });
